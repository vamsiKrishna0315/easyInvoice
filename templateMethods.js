...
class InvoiceGenerator {
    constructor() {
        ...
        this.templateMethods = {
            1: this.generateTemplate1.bind(this),
            2: this.generateTemplate2.bind(this)
        };
    }
    ...
    generateInvoice(templateId = 1) {
        const templateMethod = this.templateMethods[templateId];
        if (!templateMethod) {
    ...
    // Method for template 1
    generateTemplate1() {
      return template1(this.invoiceData);    
    }

    // Method for template 2
    generateTemplate2() {
        return `Company: ${this.invoiceData.companyName}\nItems: ${this.invoiceData.deliveryItems.join(", ")}\nPickup: ${this.invoiceData.pickupLocation}`;
    }
...
function generateAndDownloadInvoice() {
    ...
    invoiceGenerator.init(invoiceData); // Initialize with the invoice data
